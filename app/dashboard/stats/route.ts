...
export async function GET() {
    try {
        ...
        // รายได้
        const totalReevenueResult = await prisma.bill.aggregate({
            where: { status: 'paid' },
            _sum: { totalAmount: true }
        });
        const totalRevenue = totalReevenueResult._sum.totalAmount || 0;

        // 2. Room Occupancy pie chart
        const OccupancyData = [
            { name: 'มีผู้เข้าพัก', value: occupiedRoomsCount, color: '#3b82f6' },
        ...
        // 4. Monthly Revenue Bar chart (last 6 month)
        ...
        // แปลง map เป็น array และ reverse เพื่อให้เรียงจากเดือนปัจจุบัน
        const monthlyhRevenueData = Object.entries(monthlyRevenueMap)
            .map(([month, amount]) => ({ month, amount }))
            .reverse();


        // 5. Generate Distribution (Donut or pie)
        ...
        // Response to Client ใช้ ส่งข้อมูลไปใช้
